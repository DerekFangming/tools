                            ...
                            for (int i = 0; i < players.length(); i++) {
                                ...
                                if (player.getInt("level") < discordGuild.getRoleLevelRequirement()) {
                                    ...
                                    if (user.getLevelTagId() == null) {
                                        ...
                                        Role targetRole = guild.getRoleById(discordGuild.getRoleRankRoleId());
                                        guild.modifyRolePositions().selectPosition(role).moveTo(targetRole.getPosition()).queue();
                                        channel.sendMessage("<@" + member.getId() + "> tag **" + command[2].trim() + "**已创建成功。").queue();
                                        return;
                                    } else {
                                        Role role = guild.getRoleById(user.getLevelTagId());
                                        role.getManager().setName(command[2].trim()).setColor(Color.decode(color)).queue();
                                        channel.sendMessage("<@" + member.getId() + "> tag **" + command[2].trim() + "**已更新成功。").queue();
                                        return;
                                    }
                                }
                            }
